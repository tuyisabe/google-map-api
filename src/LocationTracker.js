...
const LocationTracker = ({ waypoints }) => {
    ...
    useEffect(() => {
        if (!currentLocation || !waypoints || waypoints.length === 0) return;
        ...
        if (!nextStop) {
            console.error('Next stop data not available.');
            return;
        }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: -10 }}>
            <div>
                ...
                {eta !== null && <p style={{ marginTop: '-18px' }}>Distance: {Math.round(dist / 1000)} km &nbsp;&nbsp; Time: {eta} minutes</p>}
            </div>
        </div>
    );
};
